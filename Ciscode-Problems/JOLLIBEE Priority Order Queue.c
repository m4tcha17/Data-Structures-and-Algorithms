...
QUEUE* initializeQueue(){
  ...
  Q->orderQueue = (ORDER*)malloc(MAX * sizeof(ORDER));
  Q->last = -1;
  return Q;
}
...
void priorityHeapifyDown(QUEUE **PrioQueue, int root){
  ...
  while(1){
    ...
    // Choose which child to go to, the smallest child
    if(RC <= (*PrioQueue)->last){
      child = ((*PrioQueue)->orderQueue[LC].orderNumber < (*PrioQueue)->orderQueue[RC].orderNumber) ? LC : RC;
    }
    else if(LC <= (*PrioQueue)->last){
      child = LC;
    }
    else{
      return;
    ...
    // if parent is bigger than the child, swap
    if((*PrioQueue)->orderQueue[root].orderNumber > (*PrioQueue)->orderQueue[child].orderNumber){
      swap(&((*PrioQueue)->orderQueue[root]), &((*PrioQueue)->orderQueue[child]));
      root = child;
    } 
    else{
      return;
...
void createQueue(QUEUE **PrioQueue, ORDER samples[], int size){
    ...
    for(int i = ((*PrioQueue)->last - 1) / 2; i >= 0; i--)
        priorityHeapifyDown(PrioQueue, i);
}

void priorityHeapifyUp(QUEUE **PrioQueue){
    // code here
  int child = (*PrioQueue)->last;
  int parent = (child - 1) / 2;

  while((*PrioQueue)->orderQueue[parent].orderNumber > (*PrioQueue)->orderQueue[child].orderNumber){
    swap(&((*PrioQueue)->orderQueue[parent]), &((*PrioQueue)->orderQueue[child]));
    child = parent;
    parent = (child - 1) / 2;
  }
}
...
void enqueueOrder(QUEUE **PrioQueue, ORDER newOrder){
    ...
    for(int i = 0; i <= (*PrioQueue)->last; i++){  //check duplicate
  ...
  priorityHeapifyUp(PrioQueue);
  printf("...Successfully enqueued order [%d].\n", newOrder.orderNumber);
}
...
ORDER dequeueOrder(QUEUE **PrioQueue){
    ...
    //dequeue code here
  ORDER minOrder = (*PrioQueue)->orderQueue[0];
  (*PrioQueue)->orderQueue[0] = (*PrioQueue)->orderQueue[(*PrioQueue)->last--];  
  int parent = 0;
  ...
  while(1){
    ...
    // if parent is bigger than the child, swap
    if((*PrioQueue)->orderQueue[parent].orderNumber > (*PrioQueue)->orderQueue[child].orderNumber){
      swap(&((*PrioQueue)->orderQueue[parent]), &((*PrioQueue)->orderQueue[child]));
      parent = child;
    }
    else{
...
int main(){
    ...
    switch(choice){
        ...
        case 5:
            ...
            printf("RECEIVED AND SERVED order [%d]...\n", receiveOrder1.orderNumber);
            printf("RECEIVED AND SERVED order [%d]...\n", receiveOrder2.orderNumber);
